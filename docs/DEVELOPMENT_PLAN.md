...
| 告警运营 | 🔶 状态机 / 静默 / 聚合计数 / 批量完成；**规则求值未做** |
| 测试 | ✅ **406 个**（单元 + 集成 + 契约），真实库上跑；ruff 全绿 |
...
**现状**：`app/alerts/` 只有状态机与查询，**没有产生告警的能力**。
告警表目前只能靠测试直接插入。

| 子项 | 要点 |
|---|---|
| 规则集 | 声明式数据（`DIAGNOSIS_DESIGN.md` §7 已定：规则是数据不是代码分支） |
| 求值 | 事件/指标 → 规则匹配 → 产生或累加告警 |
| 聚合去重 | 按 `aggregationKey` 累加 `count` + 提升 `lastFiredAt`，不新建 |
| 证据绑定 | **`evidenceEventIds` 必填**（F-02 红线，数据库 CHECK 已强制） |
| 分级 | 用 `EVENT_TYPE_DEFAULT_SEVERITY` + 规则覆盖 |
| 静默 | 已实现（`silence_expired` + `silencedUntil`） |
| 恢复 | 条件不再满足时置 `resolved` |

**依赖**：三类故障场景的事件夹具（任务 2 的前置）。夹具可由 B 自己造
（按 A 的上报契约构造），**不需要等 A** —— 这点很重要，否则会无谓阻塞。

**验收标准**：

1. 给定一份场景事件序列，能产生预期数量的告警（聚合生效，无重复条目）
2. 每条告警的 `evidenceEventIds` 非空且指向真实事件
3. 重复触发只累加 `count`，不新建告警
4. 静默期内不产生新告警；到期后恢复
5. 条件恢复后告警转 `resolved` 且 `resolvedAt` 被写入
6. **负向**：无证据的事件不产生告警（返回 0 条，不报错）
