...
| D-101 | 🔴 **平台层命名空间桥接缺失，升级为关键路径阻塞**：`zsvirt_resource_id` 无任何调用方，探针命名空间与 ZSvirt 命名空间之间无映射。当前平台层资源只能由探针代报；跨层关联的最后一公里依赖适配层 | ⛔ BLOCKED | 成员 B 2026-09-20 | `tests/test_cross_layer.py`、外部阻塞 X-06 |
...
| X-08 | **AI 工作负载日志规范**（路径 + 格式）| 决定 `inference.*` / `agent.*` 事件能否准确采集（A 意见 1） | ⛔ 待三方 + 命题方 |
| X-09 | **探针 `vmId` 注入方式** | 锚定 Pull 资源的先决条件（A 意见 2） | ⛔ 需并入 X-04 问命题方 |
| X-10 | **VM 内 `/var/run/docker.sock` 挂载方式** | 决定 `container.*` 事件（OOM/重启）能否采集（A 意见 3） | ⛔ 待命题方 |
...
| 外部依赖 | 0 | 0 | **10**（+X-08/09/10） |
| **合计** | **89** | **6** | **10**（+1 关键路径阻塞见 D-101） |
