...
| 字段 | 值 |
|---|---|
| 文档状态 | **ACTIVE v0.3 — 已实现**（引擎与规则集均已落地，见下"实现状态"） |
| 归属 | 成员 B 模块设计（产品文档） |
| 负责人 | 成员 B |
| 最后更新 | 2026-09-17 |
| 前置依赖 | `DATA_MODEL.md`（对象与字段）、`API_CONTRACT.md`（输入契约） |

> **实现状态**：本文档描述的引擎已全部实现并通过测试。
>
> | 设计 | 实现位置 |
> |---|---|
> | 五步流程 / 置信评分 / 三集影响范围 | `app/diagnosis/engine.py` |
> | 规则集（声明式数据） | `app/alerts/rules.py`（告警侧）、`app/diagnosis/types.py` 的 `Rule` |
> | 上下文装配与落库 | `app/diagnosis/service.py` |
> | 告警 → 诊断自动联动 | `app/diagnosis/auto.py` |
> | 三类场景端到端夹具 | `backend/tests/_scenarios.py` + `test_alert_engine.py` |
>
> 剩余的设计内、未实现项只有一项：**诊断规则集尚未填入内容**（`RuleSet.rules` 为空），
> 因此当前所有结论诚实地返回 `UNKNOWN` + 已收集证据。填规则属于
> `DEVELOPMENT_PLAN.md` 任务 4 之后的内容，不影响本设计的结构。
...
### 7.3 规则集的存放形式（**已定**，关闭 D-076）

**结论：Python 数据结构字面量**（`app/alerts/rules.py`、`app/diagnosis/types.py` 的 `Rule`），
不是 YAML 文件，也不是数据库表。

| 形式 | 取舍 |
|---|---|
| ✅ Python 字面量（**选用**） | 类型检查生效；`assert_rules_are_consistent()` 能在**导入时**校验每条规则引用的事件类型 / 根因码 / 资源类型都在已冻结枚举内；改动走同一套 review 与测试 |
| ❌ YAML | 需要额外 schema 与加载器；引用写错只能在**运行时**表现为"这条规则从不触发"，没有任何报错 —— 在告警系统里这是代价最高的静默失效 |
| ❌ 数据库表 | 运行时可改，但要引入迁移、缓存、并发一致性；比赛场景没有运营期改规则的需求 |

**关键理由**：规则的错误引用的默认后果是"静默地什么都不做"。把校验提前到导入时，
是这一选择的全部意义。
...
| 字段 | 为什么不支持 |
|---|---|
| `durationSec`（持续时间条件） | 需要跨批次状态记忆（流式窗口）。本版求值是"事件到达即求值"，做了半套会让人以为它生效 —— **声明了却不生效的条件比没有这个字段更危险**（D-087） |

若将来确实需要持续时间条件，正确做法是引入流式窗口引擎，而不是在现有求值器上打补丁。
...
| v0.3 | 2026-09-17 | 文档状态由 DRAFT 改为 **ACTIVE（已实现）**，并列出设计与实现的对应关系；新增 §7.3 规则集存放形式（关闭 D-076）与 §7.4 有意不支持的字段（`durationSec`，D-087） | 已确认 |
